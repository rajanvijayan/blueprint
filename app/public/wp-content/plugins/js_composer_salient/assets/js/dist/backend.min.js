...
window.vc || (window.vc = {}),
    ...
    function(vc, _, $) {
        ...
        window.vc_toTitleCase = function(str) {
            ...
            }), preserve_linebreaks && (text = text.replace(/<wp-line-break>/g, "\n")), text = (text = preserve_br ? text.replace(/<wp-temp-br([^>]*)>/g, "<br$1>") : text).replace(/<p>(<!--(?:.*)-->)<\/p>/g, "$1")
            ...
            GutenbergParam = function(options) {
                return this.iframe = null, this.options = options || {}, this.value = this.options && this.options.value ? this.options.value : "", this.iframeLoaded = function() {
                    var wpData = !!this.iframe.contentWindow.wp && this.iframe.contentWindow.wp.data,
                        ...
                        blockContent = this.value, // nectar addition;
                        wpBlocks = this.iframe.contentWindow.wp.blocks,  // nectar addition;
                        blockEditor = wpData.dispatch("core/block-editor"), // nectar addition;
                   ...
                   /* nectar addition */
                    var that = this;
                    setTimeout(function() {
                        editor.setupEditor(localizations), 
                        postTitle = that.iframe, 
                        postTitle = $(postTitle).contents(), 
                        notice = postTitle.find(".edit-post-header-toolbar"), 
                        wpData = getControlsHTML(), 
                        $(wpData).insertAfter(notice), 
                        wpData = vc.atts.gutenberg, 
                        postTitle.find(".wpb-gutenberg-modal-close-button").on("click", wpData.closeEditor.bind(wpData)), 
                        postTitle.find(".vc_gutenberg-modal-update-button").on("click", wpData.updateEditor.bind(wpData));

                        var parsedBlocks = '';
                        var errorParsing = false;
                       try {
                        parsedBlocks = wpBlocks.parse(blockContent);
                       } catch {
                        console.log('An error parsing the blocks occurred');
                        errorParsing = true;
                      
                       }
                        if( !errorParsing ) { 
                            blockEditor.insertBlocks(parsedBlocks);
                        }
        ...
        vc.clone_index = 1, vc.shortcode_view = Backbone.View.extend({
            ...
            changeShortcodeParams: function(model) {
                ...
                _.defer(function() {
                    ...
                    }).join(", ")) : _.isObject(param_settings.value) && !_.isArray(param_settings.value) && (inverted_value = _.invert(param_settings.value), label_value = _.isString(inverted_value[value]) ? inverted_value[value] : value), name.html("<label>" + name.find("label").text() + "</label>: " + label_value), name.show().removeClass("hidden-label")))
